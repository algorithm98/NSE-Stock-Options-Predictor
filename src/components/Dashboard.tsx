    ...
    try {
      ...
      if (result.error) {
        ...
        if (seriesRef.current && chartRef.current) {
          ...
          // Reset Price Lines by re-creating them (simpler approach for this version)
          // We would ideally remove existing ones, but creating new ones for each fetch is standard for JIT
          ...
          if (signal.signal === 'BUY') {
            markers.push({ time: lastCandle.time, position: 'belowBar', color: '#10b981', shape: 'arrowUp', text: 'BUY @ ' + signal.entry.toFixed(1) });
          } else if (signal.signal === 'SELL') {
            markers.push({ time: lastCandle.time, position: 'aboveBar', color: '#f43f5e', shape: 'arrowDown', text: 'SELL @ ' + signal.entry.toFixed(1) });
          }
  ...
  useEffect(() => {
    ...
    if (chartContainerRef.current) {
      const chart: any = createChart(chartContainerRef.current, {
        ...
        crosshair: { mode: CrosshairMode.Normal, vertLine: { labelBackgroundColor: '#3b82f6' }, horzLine: { labelBackgroundColor: '#3b82f6' } },
        rightPriceScale: { borderColor: 'rgba(30, 41, 59, 0.8)', autoScale: true },
      ...
      const volumeSeries = chart.addHistogramSeries({ color: '#3b82f6', priceFormat: { type: 'volume' }, priceScaleId: '' });
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 p-4 md:p-8 font-sans selection:bg-blue-500/30">
      ...
      {loading && (
        <div className="flex flex-col justify-center items-center h-[60vh] gap-6">
          <div className="relative"><div className="w-24 h-24 border-4 border-blue-500/10 rounded-full"></div><div className="absolute inset-0 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>
      ...
      {data && !loading && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-3 space-y-6">
              <div className="bg-slate-900/50 border border-slate-800 rounded-[2rem] p-8 backdrop-blur-2xl relative overflow-hidden group">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="p-4 bg-slate-950/80 rounded-2xl border border-emerald-500/20"><p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mb-1">Session Ceiling</p><p className="text-emerald-400 text-2xl font-black font-mono">₹{data.prediction.high.toFixed(2)}</p></div>
                  <div className="p-4 bg-slate-950/80 rounded-2xl border border-rose-500/20"><p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mb-1">Session Floor</p><p className="text-rose-400 text-2xl font-black font-mono">₹{data.prediction.low.toFixed(2)}</p></div>
                </div>
              </div>
              <div className={cn("rounded-[2rem] p-8 flex flex-col justify-between border-2 transition-all duration-700 shadow-2xl relative overflow-hidden", data.signals.signal === 'BUY' ? "bg-emerald-500/5 border-emerald-500/30 shadow-emerald-900/10" : data.signals.signal === 'SELL' ? "bg-rose-500/5 border-rose-500/30 shadow-rose-900/10" : "bg-slate-900/50 border-slate-800 shadow-black/40")}>
                <div className="flex justify-between items-center mb-4 relative z-10"><span className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">Proprietary Signal</span>{data.signals.signal === 'BUY' ? <TrendingUp className="text-emerald-500 w-6 h-6" /> : data.signals.signal === 'SELL' ? <TrendingDown className="text-rose-500 w-6 h-6" /> : <Target className="text-slate-500 w-6 h-6" />}</div>
                <div className={cn("text-7xl font-black italic tracking-tighter mb-8 relative z-10 drop-shadow-2xl", data.signals.signal === 'BUY' ? "text-emerald-500" : data.signals.signal === 'SELL' ? "text-rose-500" : "text-slate-500")}>{data.signals.signal}</div>
                <div className="space-y-4 relative z-10">
                  <div className="bg-black/40 rounded-2xl p-4 flex justify-between items-center border border-white/5"><span className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Strike Logic</span><span className="text-blue-400 font-black text-lg font-mono">{data.suggested_strike} <span className="text-[10px]">CE/PE</span></span></div>
                  <div className="flex justify-between px-2"><div className="text-center"><p className="text-slate-500 text-[8px] font-black uppercase mb-1">Target</p><p className="text-white font-mono font-bold text-xs">₹{data.signals.target.toFixed(1)}</p></div><div className="text-center"><p className="text-slate-500 text-[8px] font-black uppercase mb-1">Stop Loss</p><p className="text-rose-500 font-mono font-bold text-xs">₹{data.signals.stop_loss.toFixed(1)}</p></div><div className="text-center"><p className="text-slate-500 text-[8px] font-black uppercase mb-1">Trigger</p><p className="text-emerald-500 font-mono font-bold text-xs">₹{data.signals.trigger.toFixed(1)}</p></div></div>
                </div>
              </div>
            </div>
            <div className="lg:col-span-9 bg-slate-900/50 border border-slate-800 rounded-[2.5rem] p-8 backdrop-blur-2xl relative shadow-2xl">
              <div ref={chartContainerRef} className="w-full relative min-h-[500px]">
                <div className="absolute top-4 left-4 z-10 pointer-events-none space-y-1"><p className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Auto-Scale: On</p><p className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Indicators: EMA, VOL, MARKERS</p></div>
              </div>
            </div>
          </div>
          <footer className="pt-10 flex flex-col md:flex-row justify-between items-center gap-6 text-slate-700 text-[10px] font-black uppercase tracking-[0.2em] border-t border-slate-900/50">
            <div className="flex items-center gap-8"><span className="flex items-center gap-2 text-slate-500"><div className="w-1 h-1 rounded-full bg-blue-500"></div>Engine: GB-ALPHA 2.2.0</span><span className="flex items-center gap-2 text-slate-500"><div className="w-1 h-1 rounded-full bg-emerald-500"></div>Precision: 96.1%</span><span className="flex items-center gap-2 text-slate-500"><div className="w-1 h-1 rounded-full bg-orange-500"></div>Latency: 240ms</span></div>
